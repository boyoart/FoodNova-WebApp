...
from models import (
    ...
    ComingSoonSubscriber as DBComingSoonSubscriber,
...
def dispatch_test_mode_enabled() -> bool:
    enabled = str(os.getenv("DISPATCH_TEST_MODE") or "").strip().lower() in {"1", "true", "yes", "on"}
    if enabled:
        print("DISPATCH_TEST_MODE_ENABLED", json_dump({
            "ignore_geofence": True,
            "ignore_rider_to_pickup_distance": True,
...
def _create_broadcast_notification(title: str, message: str, notif_type: str = "broadcast",
    ...
    try:
        ...
        for email in customer_emails:
            ...
            recipient_count += 1
        db.commit()
        for token, notification_id, email in push_targets:
...
def subscribe_coming_soon(payload: ComingSoonSubscriberPayload):
    ...
    try:
        print("COMING_SOON_REQUEST", json_dump({
            "route": "/coming-soon/subscribe",
            "timestamp": iso(datetime.utcnow()),
        }))
        print("COMING_SOON_PAYLOAD", json_dump({
            "email": email,
            "source": "coming_soon_page",
            "timestamp": iso(datetime.utcnow()),
        }))
        print("COMING_SOON_SUBSCRIBE_REQUEST", json_dump({
            "email": email,
            "source": "coming_soon_page",
            "timestamp": iso(datetime.utcnow()),
        }))
        subscriber = db.query(DBComingSoonSubscriber).filter(func.lower(DBComingSoonSubscriber.email) == email).first()
        duplicate = subscriber is not None
        if not subscriber:
            subscriber = DBComingSoonSubscriber(email=email, source="coming_soon_page")
            db.add(subscriber)
            db.flush()
        db.commit()
        print("COMING_SOON_SAVE_SUCCESS", json_dump({
            "email": email,
            "subscriber_id": subscriber.id,
            "duplicate": duplicate,
            "timestamp": iso(datetime.utcnow()),
        }))
        print("COMING_SOON_SUBSCRIBE_SUCCESS", json_dump({
            "email": email,
            "subscriber_id": subscriber.id,
            "duplicate": duplicate,
            "timestamp": iso(datetime.utcnow()),
        ...
        db.rollback()
        print("COMING_SOON_SAVE_FAILED", json_dump({
            "email": email,
            "error_type": type(error).__name__,
            "message": str(error),
            "timestamp": iso(datetime.utcnow()),
        ...
        log_backend_exception(
            "COMING_SOON_SUBSCRIBE_FAILED",
            error,
            route="/coming-soon/subscribe",
            email=email,
            source="coming_soon_page",
        )
        print("COMING_SOON_SUBSCRIBE_FAILED", json_dump({
            "email": email,
            "error_type": type(error).__name__,
            "message": str(error),
            "timestamp": iso(datetime.utcnow()),
        ...
        return JSONResponse(
            status_code=500,
            content={"success": False, "detail": "Could not save email. Please try again."},
        )
    finally:
        db.close()
        ...
        items = [
            {
                "id": item.id,
                "email": item.email,
                "source": item.source or "coming_soon_page",
...
def get_firebase_app():
    if not firebase_admin:
        print("PUSH_FAILED reason=firebase_admin_unavailable")
        return None
    ...
    print(
        "FCM_CREDENTIALS "
        f"service_account_json={'present' if service_account_json else 'missing'} "
        f"service_account_path={'present' if service_account_path else 'missing'}"
    )
    try:
        ...
        print("PUSH_FAILED reason=firebase_init_error error=", repr(error))
    return None
...
def send_delivery_offer_push(worker: DBDeliveryWorker, offer: DBDeliveryOffer) -> None:
    ...
    for token in json_load(getattr(worker, "fcm_tokens_json", None), []) or []:
        if token and token not in tokens:
            tokens.append(token)
    print("ORDER_OFFER_PUSH_START", json_dump({
        "worker_id": worker.id,
    ...
    if not tokens:
        print("RIDER_FCM_TOKEN_MISSING", json_dump({
            "worker_id": worker.id,
            "user_id": worker.user_id,
            "offer_id": offer.id,
            "order_id": offer.order_id,
        ...
        return
    print("RIDER_FCM_TOKEN_FOUND", json_dump({
        "worker_id": worker.id,
        "user_id": worker.user_id,
        "offer_id": offer.id,
        "order_id": offer.order_id,
    ...
    for token in tokens:
        ...
        print(("ORDER_OFFER_PUSH_SENT" if result.get("success") else "ORDER_OFFER_PUSH_FAILED"), json_dump({
            "worker_id": worker.id,
            "user_id": worker.user_id,
            "offer_id": offer.id,
            "order_id": offer.order_id,
...
def profile_to_dict(profile: DBProfile, user: DBUser = None) -> dict:
    ...
    return {
        ...
        "role": user.role or "customer" if user else "customer",
        "admin_role": normalize_admin_role(getattr(user, "admin_role", None)) if user and (user.role or "") == "admin" else "",
        "permissions": get_admin_permissions(user) if user and (user.role or "") == "admin" else [],
...
def worker_to_dict(worker: DBDeliveryWorker) -> dict:
    ...
    return {
        ...
        "rider_name": worker.full_name or "",
        "rider_id": f"FN-RDR-{worker.id}",
        ...
        "wallet_enabled": bool(rider_earnings_enabled() and lifecycle_status == "ACTIVE"),
...
def order_to_dict(order: DBOrder) -> dict:
    ...
    return {
        ...
        "rider_display_id": f"FN-RDR-{getattr(order, 'rider_id', None)}" if getattr(order, "rider_id", None) else "",
        "rider_name": getattr(order, "rider_name", "") or "",
        "assigned_rider_name": getattr(order, "rider_name", "") or "",
        "rider_phone": getattr(order, "rider_phone", "") or "",
        "assigned_rider_phone": getattr(order, "rider_phone", "") or "",
        ...
        "assigned_worker_type": getattr(order, "delivery_worker_type", "") or "",
...
def order_assignment_snapshot(order: DBOrder, db=None) -> dict:
    ...
    if db is not None and worker_id:
        try:
            ...
            worker = None
    rider_name = (getattr(worker, "full_name", "") if worker else "") or getattr(order, "rider_name", "") or ""
    rider_phone = (getattr(worker, "phone", "") if worker else "") or getattr(order, "rider_phone", "") or ""
    vehicle_type = (getattr(worker, "vehicle_type", "") if worker else "") or getattr(order, "rider_vehicle_type", "") or ""
    vehicle_number = (getattr(worker, "plate_number", "") if worker else "") or getattr(order, "rider_vehicle_number", "") or ""
    photo_url = rider_profile_photo_url(worker) if worker else ""
    photo_url = photo_url or getattr(order, "rider_photo_url", "") or ""
    worker_type = (getattr(worker, "worker_type", "") if worker else "") or getattr(order, "delivery_worker_type", "") or ""
    return {
        "delivery_worker_id": worker_id,
        "rider_id": worker_id,
        "rider_display_id": f"FN-RDR-{worker_id}" if worker_id else "",
        "rider_name": rider_name,
        "assigned_rider_name": rider_name,
        "rider_phone": rider_phone,
        "assigned_rider_phone": rider_phone,
        "rider_vehicle_type": vehicle_type,
        "rider_vehicle_number": vehicle_number,
        "rider_photo_url": photo_url,
        "rider_photo": photo_url,
        "profile_photo_url": photo_url,
        "delivery_worker_type": worker_type,
        "assigned_worker_type": worker_type,
        "delivery_status": getattr(order, "delivery_status", "") or "",
        "dispatch_status": canonical_dispatch_status(order),
        "delivery_assigned_at": iso(getattr(order, "delivery_assigned_at", None)),
    }
...
def order_to_dict_for_context(order: DBOrder, db=None, context: str = "") -> dict:
    ...
    if context == "admin":
        lookup_payload = {
            ...
            "assigned_rider_name": data.get("assigned_rider_name"),
            "assigned_rider_phone": data.get("assigned_rider_phone"),
            "dispatch_status": data.get("dispatch_status"),
            "delivery_status": data.get("delivery_status"),
        }
        print("ADMIN_ASSIGNMENT_LOOKUP", json_dump(lookup_payload))
        print("ADMIN_ORDER_ASSIGNMENT_LOOKUP", json_dump({
            "order_id": lookup_payload.get("order_id"),
            "rider_id": lookup_payload.get("rider_id"),
            "delivery_worker_id": lookup_payload.get("delivery_worker_id"),
            "worker_name": lookup_payload.get("assigned_rider_name"),
        }))
        print("ADMIN_ORDER_ASSIGNMENT_RESULT", json_dump({
            "order_id": lookup_payload.get("order_id"),
            "rider_id": lookup_payload.get("rider_id"),
            "delivery_worker_id": lookup_payload.get("delivery_worker_id"),
            "worker_name": lookup_payload.get("assigned_rider_name"),
            "assigned": bool(lookup_payload.get("delivery_worker_id") or lookup_payload.get("rider_id")),
        }))
    return data
...
def get_foodnova_pickup_coordinates(db) -> tuple[Optional[float], Optional[float]]:
    lat = os.getenv("FOODNOVA_PICKUP_LATITUDE") or os.getenv("FOODNOVA_PICKUP_LAT") or ""
    lng = os.getenv("FOODNOVA_PICKUP_LONGITUDE") or os.getenv("FOODNOVA_PICKUP_LNG") or os.getenv("FOODNOVA_PICKUP_LON") or ""
    if lat and lng:
        try:
            return float(lat), float(lng)
        except (TypeError, ValueError):
...
def canonical_dispatch_status(order: DBOrder) -> str:
    ...
    if raw_delivery in {"arrived", "arrived_at_customer"}:
        return "ARRIVED"
    if raw_delivery in {"arrived_at_pickup", "at_pickup"}:
...
def tracking_route_service(rider_lat: float, rider_lng: float, customer_lat: float, customer_lng: float) -> dict:
    ...
    if google_key:
        ...
        try:
            ...
            if distance_meters_value is not None and duration_seconds is not None and polyline:
                print("TRACKING_ROUTE_PROVIDER", json_dump({"provider": "google_directions", "status": body.get("status") or "OK"}))
            ...
            print("TRACKING_ROUTE_ERROR", json_dump({
                "provider": "google_directions",
                "status": body.get("status") or "NO_ROUTE",
                "error_message": body.get("error_message", ""),
            }))
    ...
    try:
        ...
        if route.get("distance") is not None and route.get("duration") is not None and coordinates:
            print("TRACKING_ROUTE_PROVIDER", json_dump({"provider": "osrm", "status": body.get("code") or "Ok"}))
            print("TRACKING_ROUTE_CREATED", json_dump({
                "provider": "osrm",
...
def order_rider_location_payload(order: DBOrder, db) -> dict:
    ...
    pickup_lat, pickup_lng = get_foodnova_pickup_coordinates(db)
    ...
    pickup_valid = valid_tracking_coordinate(pickup_lat, pickup_lng)
    ...
    origin_valid, destination_valid = rider_valid, customer_valid
    if dispatch_status in {"ASSIGNED", "ACCEPTED"}:
        route_stage = "rider_to_pickup"
        destination_lat, destination_lng = pickup_lat, pickup_lng
        destination_valid = pickup_valid
    elif dispatch_status == "ARRIVED_AT_PICKUP":
        route_stage = "pickup_to_customer"
        origin_lat, origin_lng = pickup_lat, pickup_lng
        origin_valid = pickup_valid
    print("TRACKING_STAGE_CHANGED", json_dump({
        "order_id": order.id,
        "dispatch_status": dispatch_status,
        "route_stage": route_stage,
    }))
    if origin_valid and destination_valid:
        route = tracking_route_service(float(origin_lat), float(origin_lng), float(destination_lat), float(destination_lng))
        ...
        print(("ROUTE_TO_PICKUP_CREATED" if route_stage == "rider_to_pickup" else "ROUTE_TO_CUSTOMER_CREATED"), json_dump({
            "order_id": order.id,
            "route_stage": route_stage,
    ...
    print("CUSTOMER_TRACKING_LOOKUP", json_dump({
        "order_id": order.id,
        "order_code": order.order_code,
        "delivery_worker_id": getattr(order, "delivery_worker_id", None),
        "rider_id": getattr(order, "rider_id", None),
        "worker_found": bool(worker),
        "destination_found": customer_valid,
        "pickup_found": pickup_valid,
        "route_stage": route_stage,
        "dispatch_status": dispatch_status,
    ...
    print("TRACKING_RIDER_FOUND", json_dump({
        "order_id": order.id,
        "worker_id": worker_id,
        "found": bool(worker),
        "rider_name": (worker.full_name if worker else None) or getattr(order, "rider_name", "") or "",
    }))
    print("TRACKING_DESTINATION_FOUND", json_dump({
        "order_id": order.id,
        "found": customer_valid,
    ...
    print("TRACKING_MAP_DATA", json_dump({
        "order_id": order.id,
        "tracking_visible": tracking_visible,
        "tracking_available": tracking_available,
        "route_provider": route_provider,
        "route_status": route_status,
        "route_stage": route_stage,
        "rider_has_coordinates": rider_valid,
        "destination_has_coordinates": customer_valid,
        "pickup_has_coordinates": pickup_valid,
        "polyline_points": len(route_polyline),
    }))
    print("TRACKING_RIDER_COORDS", json_dump({
        "order_id": order.id,
        "rider_id": worker_id,
    ...
    print("TRACKING_DESTINATION_COORDS", json_dump({
        "order_id": order.id,
    ...
    print("TRACKING_POLYLINE_POINTS", json_dump({
        "order_id": order.id,
        "count": len(route_polyline),
        "first": route_polyline[0] if route_polyline else None,
        "last": route_polyline[-1] if route_polyline else None,
    }))
    ...
    return {
        ...
        "rider": {
            ...
            "rider_id": f"FN-RDR-{worker.id if worker else worker_id}" if (worker or worker_id) else "",
        ...
        "pickup": {
            "latitude": float(pickup_lat) if pickup_valid else None,
            "longitude": float(pickup_lng) if pickup_valid else None,
            "address": "FoodNova pickup",
        },
        "route_destination": {
            "latitude": float(destination_lat) if destination_valid else None,
            "longitude": float(destination_lng) if destination_valid else None,
            "type": "pickup" if route_stage == "rider_to_pickup" else "customer",
        },
        "route_stage": route_stage,
        "distance_meters": distance_remaining,
        "eta_minutes": eta_minutes,
...
def worker_inside_zone(worker: DBDeliveryWorker, latitude: float, longitude: float, db) -> bool:
    if dispatch_test_mode_enabled():
        return True
    if normalize_worker_type(worker.worker_type) != "messenger":
        return False
    zone = ensure_default_operational_zone(db)
...
def worker_assignment_counts(db, worker: DBDeliveryWorker, exclude_offer_id: Optional[int] = None) -> dict:
    ...
    if exclude_offer_id:
        offer_query = offer_query.filter(DBDeliveryOffer.id != exclude_offer_id)
    active_order_count = active_order_filter(db.query(DBOrder)).filter(
    ...
    return {
        "active_offer_count": offer_query.count(),
        "active_order_count": active_order_count,
    }


def worker_offer_eligibility_reason(
    db,
    worker: DBDeliveryWorker,
    delivery_type: str,
    ...
    test_mode = dispatch_test_mode_enabled()
    worker_type = normalize_worker_type(worker.worker_type, default="")
    age = worker_gps_age_seconds(worker)
    assignment_counts = worker_assignment_counts(db, worker, exclude_offer_id)
    print("WORKER_ELIGIBILITY_START", json_dump({
        "worker_id": worker.id,
        "worker_type": worker.worker_type,
        "normalized_worker_type": worker_type,
        ...
        "last_seen_at": iso(worker.last_seen_at),
        "test_mode": test_mode,
        "active_offer_count": assignment_counts.get("active_offer_count"),
        "active_order_count": assignment_counts.get("active_order_count"),
    }))
    print("WORKER_TYPE_VALUE", json_dump({
        "worker_id": worker.id,
        "worker_type": worker.worker_type,
        "normalized_worker_type": worker_type,
    }))
    print("WORKER_STATUS_VALUE", json_dump({
        "worker_id": worker.id,
        "kyc_status": worker.kyc_status,
        "lifecycle_status": rider_lifecycle_status(worker),
    }))
    print("WORKER_OPERATIONAL_STATUS", json_dump({
        "worker_id": worker.id,
        "operational_status": worker.operational_status,
    }))
    print("WORKER_LAST_GPS", json_dump({
        "worker_id": worker.id,
    ...
    if worker.id in excluded_worker_ids:
        reason = "worker_previously_declined_or_expired_offer"
        print("WORKER_REJECTION_REASON", json_dump({**rejection_payload, "reason": reason}))
        return False, reason
    access_reason = delivery_worker_access_block_reason(worker)
    if access_reason:
        reason = f"access_blocked_{access_reason}"
        print("WORKER_REJECTION_REASON", json_dump({**rejection_payload, "reason": reason}))
        return False, reason
    ...
    if rider_lifecycle_status(worker) != "ACTIVE":
        reason = f"lifecycle_not_active_{rider_lifecycle_status(worker)}"
        print("WORKER_REJECTION_REASON", json_dump({**rejection_payload, "reason": reason}))
        return False, reason
    if str(worker.operational_status or "").upper() != "ONLINE":
        reason = f"not_online_{worker.operational_status or 'missing'}"
        print("WORKER_REJECTION_REASON", json_dump({**rejection_payload, "reason": reason}))
        return False, reason
    if assignment_counts.get("active_offer_count", 0) > 0:
        reason = "has_active_assignment_or_pending_offer"
        print("WORKER_REJECTION_REASON", json_dump({**rejection_payload, "reason": reason}))
        return False, reason
    if assignment_counts.get("active_order_count", 0) > 0:
        reason = "has_active_order"
        print("WORKER_REJECTION_REASON", json_dump({**rejection_payload, "reason": reason}))
        return False, reason
    if age is None:
        reason = "missing_gps_ping"
        print("WORKER_REJECTION_REASON", json_dump({**rejection_payload, "reason": reason}))
        return False, reason
    if not test_mode and worker_type == "messenger" and delivery_type != "short_distance":
        reason = f"messenger_not_allowed_for_{delivery_type}"
        print("WORKER_REJECTION_REASON", json_dump({**rejection_payload, "reason": reason}))
        return False, reason
    if not test_mode and worker_type == "messenger" and not bool(worker.inside_zone):
        reason = "messenger_outside_geofence"
        print("WORKER_REJECTION_REASON", json_dump({**rejection_payload, "reason": reason}))
        return False, reason
    if not test_mode and worker_type == "messenger" and age > GPS_RECENCY_SECONDS:
        reason = f"messenger_gps_stale_{round(age, 2)}s"
        print("WORKER_REJECTION_REASON", json_dump({**rejection_payload, "reason": reason}))
        return False, reason
    if not test_mode and worker_type == "rider" and age > 120:
        reason = f"rider_gps_stale_{round(age, 2)}s"
        print("WORKER_REJECTION_REASON", json_dump({**rejection_payload, "reason": reason}))
        return False, reason
    if worker_type not in VALID_DELIVERY_WORKER_TYPES:
        reason = f"invalid_worker_type_{worker.worker_type or 'missing'}"
        print("WORKER_REJECTION_REASON", json_dump({**rejection_payload, "reason": reason}))
        return False, reason
...
def worker_eligible_for_offer(db, worker: DBDeliveryWorker, delivery_type: str, exclude_offer_id: Optional[int] = None) -> bool:
    eligible, reason = worker_offer_eligibility_reason(db, worker, delivery_type, exclude_offer_id)
    payload = {
        "worker_id": worker.id,
        "worker_type": worker.worker_type,
        "normalized_worker_type": normalize_worker_type(worker.worker_type, default=""),
        "delivery_type": delivery_type,
        "reason": reason,
    }
    print(("WORKER_ELIGIBILITY_PASSED" if eligible else "WORKER_ELIGIBILITY_REJECTED"), json_dump(payload))
    return eligible
...
def find_available_delivery_worker(db, delivery_type: str, excluded_worker_ids: set[int] = None) -> Optional[DBDeliveryWorker]:
    ...
    if test_mode:
        type_order = ["messenger", "rider"]
    print("WORKER_CANDIDATE_SCAN_START", json_dump({
        "delivery_type": delivery_type,
        "type_order": type_order,
        "excluded_worker_ids": sorted(excluded_worker_ids),
        "test_mode": test_mode,
    }))
    total_online_workers = db.query(DBDeliveryWorker).filter(DBDeliveryWorker.operational_status == "ONLINE").count()
    total_active_workers = db.query(DBDeliveryWorker).filter(
    ...
    for worker_type in type_order:
        ...
        candidate_ids.update(worker.id for worker in workers)
        print("WORKER_QUERY_RESULT", json_dump({
            "delivery_type": delivery_type,
            "candidate_pool": worker_type,
            "count": len(workers),
            "worker_ids": [worker.id for worker in workers],
        }))
        for worker in workers:
            print("WORKER_CANDIDATE_FOUND", json_dump({
                "worker_id": worker.id,
                "candidate_pool": worker_type,
                "worker_type": worker.worker_type,
                "operational_status": worker.operational_status,
            ...
            normalized_type = normalize_worker_type(worker.worker_type, default="")
            if normalized_type != worker_type:
                mismatch_payload = {
                    "worker_id": worker.id,
                    "worker_type": worker.worker_type,
                    "normalized_worker_type": normalized_type,
                    "delivery_type": delivery_type,
                    "reason": f"candidate_pool_type_mismatch_expected_{worker_type}",
                    "candidate_pool": worker_type,
                }
                print("WORKER_REJECTION_REASON", json_dump(mismatch_payload))
                print("WORKER_ELIGIBILITY_REJECTED", json_dump(mismatch_payload))
                rejection_reasons.append(mismatch_payload)
                continue
            evaluated_ids.add(worker.id)
            eligible, reason = worker_offer_eligibility_reason(db, worker, delivery_type, excluded_worker_ids=excluded_worker_ids)
            eligibility_payload = {
                "worker_id": worker.id,
                "worker_type": worker.worker_type,
                "normalized_worker_type": normalize_worker_type(worker.worker_type, default=""),
            ...
            print(("WORKER_ELIGIBILITY_PASSED" if eligible else "WORKER_ELIGIBILITY_REJECTED"), json_dump(eligibility_payload))
            if eligible:
                eligible_ids.append(worker.id)
                print("MATCHING_CANDIDATE_SUMMARY", json_dump({
                    "delivery_type": delivery_type,
                    "total_online_workers": total_online_workers,
                    "total_active_workers": total_active_workers,
                    "total_candidate_workers": len(candidate_ids),
                    "final_eligible_workers": eligible_ids,
                    "selected_worker_id": worker.id,
                    "rejection_reasons": rejection_reasons[-10:],
                }))
                return worker
            rejection_reasons.append(eligibility_payload)
    online_workers = db.query(DBDeliveryWorker).filter(
        DBDeliveryWorker.operational_status == "ONLINE",
    ).order_by(DBDeliveryWorker.last_seen_at.desc(), DBDeliveryWorker.id.asc()).all()
    print("WORKER_QUERY_RESULT", json_dump({
        "delivery_type": delivery_type,
        "candidate_pool": "all_online_fallback",
        "count": len(online_workers),
        "worker_ids": [worker.id for worker in online_workers],
    }))
    for worker in online_workers:
        if worker.id in evaluated_ids:
            continue
        normalized = normalize_worker_type(worker.worker_type, default="")
        print("WORKER_CANDIDATE_FOUND", json_dump({
            "worker_id": worker.id,
            "candidate_pool": "all_online_fallback",
            "worker_type": worker.worker_type,
            "normalized_worker_type": normalized,
            "operational_status": worker.operational_status,
            "kyc_status": worker.kyc_status,
            "last_seen_at": iso(worker.last_seen_at),
        }))
        eligible, reason = worker_offer_eligibility_reason(db, worker, delivery_type, excluded_worker_ids=excluded_worker_ids)
        pool_reason = reason if normalized in type_order else f"not_in_candidate_type_order_{','.join(type_order) or 'none'}; {reason}"
        fallback_payload = {
            "worker_id": worker.id,
            "worker_type": worker.worker_type,
            "normalized_worker_type": normalized,
            "delivery_type": delivery_type,
            "reason": pool_reason,
            "candidate_pool": "outside_type_order",
        }
        if not (eligible and normalized in type_order):
            print("WORKER_REJECTION_REASON", json_dump(fallback_payload))
        print(("WORKER_ELIGIBILITY_PASSED" if eligible and normalized in type_order else "WORKER_ELIGIBILITY_REJECTED"), json_dump(fallback_payload))
        if eligible and normalized in type_order:
            eligible_ids.append(worker.id)
            print("MATCHING_CANDIDATE_SUMMARY", json_dump({
                "delivery_type": delivery_type,
                "total_online_workers": total_online_workers,
            ...
            return worker
        rejection_reasons.append(fallback_payload)
    print("MATCHING_CANDIDATE_SUMMARY", json_dump({
        "delivery_type": delivery_type,
        "total_online_workers": total_online_workers,
        "total_active_workers": total_active_workers,
        ...
        "rejection_reasons": rejection_reasons[-25:],
    }))
    return None
...
def create_delivery_offer_for_order(db, order: DBOrder, worker: DBDeliveryWorker, offer_type: str = "automatic") -> DBDeliveryOffer:
    ...
    socket_targets = [
        ("delivery_offer_created", f"user:{worker.user_id}"),
        ("delivery_offer_created", f"dispatch:{worker.user_id}"),
        ("delivery:offer", f"user:{worker.user_id}"),
        ("dispatch:offer", f"user:{worker.user_id}"),
        ("delivery:offer", f"dispatch:{worker.user_id}"),
        ("dispatch:offer", f"dispatch:{worker.user_id}"),
        ("dispatch:offer_created", "role:admin"),
    ]
    for event_name, room_name in socket_targets:
        if event_name == "delivery_offer_created":
            print("SOCKET_DELIVERY_OFFER_EMITTED", json_dump({
                "event": event_name,
                "room": room_name,
                "offer_id": offer.id,
                "order_id": order.id,
        ...
        print("ORDER_OFFER_SOCKET_EMIT", json_dump({
            "event": event_name,
            "room": room_name,
            "offer_id": offer.id,
            "order_id": order.id,
...
def start_delivery_matching(db, order: DBOrder, request: Request = None) -> Optional[DBDeliveryOffer]:
    ...
    if (order.delivery_method or "delivery") != "delivery":
        print("ORDER_MATCHING_BLOCKED", json_dump({"order_id": order.id, "reason": "not_delivery"}))
        print("ORDER_MATCHING_REASON", json_dump({"order_id": order.id, "reason": "not_delivery"}))
    ...
    if not ready:
        ...
        print("ORDER_MATCHING_REASON", json_dump({
            "order_id": order.id,
            "reason": ready_reason,
        }))
        return None
...
DISPATCH_ACTIVE_DELIVERY_STATUSES = {"ASSIGNED", "ACCEPTED", "ARRIVED_AT_PICKUP", "PICKED_UP", "IN_TRANSIT", "ARRIVED"}
DISPATCH_HISTORY_DELIVERY_STATUSES = {"DELIVERED", "COMPLETED", "CANCELLED", "CANCELED"}
DELIVERY_STATUS_API_TO_DB = {
    "assigned": "ACCEPTED",
    "accepted": "ACCEPTED",
...
def normalize_delivery_status_transition(raw_status: str) -> tuple[str, str]:
    ...
    if normalized == "delivered":
        raise HTTPException(status_code=400, detail="Delivery completion requires the customer's PIN.")
...
def ensure_dispatch_eligible_admin_status(order: DBOrder) -> bool:
    if str(getattr(order, "payment_status", "") or "").strip().lower() not in DELIVERY_MATCH_CONFIRMED_PAYMENTS:
        return False
    if (getattr(order, "delivery_method", None) or "delivery") != "delivery":
        return False
    dispatch_admin_statuses = {"order_placed", "pending_payment", "receipt_submitted", "payment_confirmed", "", None}
    changed = False
    if getattr(order, "status", None) in dispatch_admin_statuses:
        order.status = "processing"
        changed = True
    if getattr(order, "order_status", None) in dispatch_admin_statuses:
        order.order_status = "processing"
        changed = True
    if getattr(order, "fulfillment_status", None) in dispatch_admin_statuses:
    ...
    if changed:
        ...
        print("ADMIN_ORDER_ACTION", json_dump({
            "order_id": order.id,
            "order_code": order.order_code,
            "action": "auto_promote_payment_confirmed_for_dispatch",
...
def delivery_order_ready_for_matching(order: DBOrder) -> tuple[bool, str]:
    ...
    if delivery_status.upper() in DISPATCH_ACTIVE_DELIVERY_STATUSES.union(DISPATCH_HISTORY_DELIVERY_STATUSES):
...
def auto_match_ready_delivery_orders(db, request: Request = None, reason: str = "unspecified", limit: int = 50) -> dict:
    ...
    for order in orders:
        stats["scanned"] += 1
        if ensure_dispatch_eligible_admin_status(order):
            db.flush()
        ready, ready_reason = delivery_order_ready_for_matching(order)
        if not ready:
            stats["skipped"] += 1
            ...
            continue
        stats["eligible"] += 1
        print("ORDER_READY_FOR_DISPATCH", json_dump({
            "order_id": order.id,
            "order_code": order.order_code,
            "reason": ready_reason,
            "payment_status": getattr(order, "payment_status", ""),
...
def apply_dispatch_assignment_to_order(
    db,
    order: DBOrder,
    worker: DBDeliveryWorker,
    delivery_status: str = "ACCEPTED",
    delivery_note: str = "",
    mark_out_for_delivery: bool = False,
) -> dict:
    """Single write path for assigning an order to a dispatch worker."""
    ...
    order.delivery_note = delivery_note or getattr(order, "delivery_note", "") or ""
    order.delivery_assigned_at = order.delivery_assigned_at or datetime.utcnow()
    if mark_out_for_delivery:
    ...
    db.flush()
    snapshot = order_assignment_snapshot(order, db)
    print("ORDER_ASSIGNMENT_RESULT", json_dump({
        "order_id": order.id,
        "order_code": order.order_code,
        "delivery_worker_id": snapshot.get("delivery_worker_id"),
        "rider_id": snapshot.get("rider_id"),
        "worker_name": snapshot.get("assigned_rider_name"),
        "delivery_status": snapshot.get("delivery_status"),
        "dispatch_status": snapshot.get("dispatch_status"),
        "mark_out_for_delivery": bool(mark_out_for_delivery),
    }))
    return snapshot
...
def assign_delivery_offer_to_order(db, offer: DBDeliveryOffer, worker: DBDeliveryWorker, order: DBOrder, request: Request, actor: dict, automatic: bool = False) -> dict:
    print("ORDER_ASSIGNMENT_START", json_dump({
        "offer_id": offer.id,
        "order_id": order.id,
        "order_code": order.order_code,
        "worker_id": worker.id,
        "worker_name": worker.full_name,
        "previous_delivery_worker_id": getattr(order, "delivery_worker_id", None),
        "previous_rider_id": getattr(order, "rider_id", None),
        "previous_delivery_status": getattr(order, "delivery_status", ""),
    }))
    ...
    print("ORDER_ASSIGNMENT_SUCCESS", json_dump({
        ...
        "order_id": order.id,
        "order_code": order.order_code,
        "delivery_worker_id": assignment_snapshot.get("delivery_worker_id"),
        "rider_id": assignment_snapshot.get("rider_id"),
        "assigned_rider_name": assignment_snapshot.get("assigned_rider_name"),
        "assigned_rider_phone": assignment_snapshot.get("assigned_rider_phone"),
        "dispatch_status": assignment_snapshot.get("dispatch_status"),
        "delivery_status": assignment_snapshot.get("delivery_status"),
    }))
    order_data = order_to_dict_for_context(order, db)
    ...
    socket_emit("admin:orders", {"order_id": order.id, "order": order_data, "event": "order_assigned"}, room="role:admin")
    return order_data
...
def sync_rider_onboarding_state(db, worker: DBDeliveryWorker, actor: dict = None, note: str = "") -> str:
    ...
    if rider:
        ...
        rider.wallet_enabled = bool(rider_earnings_enabled() and stage == "approved")
...
def rider_detail_payload(db, worker: DBDeliveryWorker) -> dict:
    ...
    return {
        ...
        "rider": {
            ...
            "wallet_enabled": bool(rider_earnings_enabled() and rider and rider.wallet_enabled),
            "earnings_enabled": rider_earnings_enabled(),
...
def ensure_database_compatibility():
    ...
    column_specs = {
        ...
        "coming_soon_subscribers": {
            "email": "VARCHAR(150)",
            "source": "VARCHAR(80) DEFAULT 'coming_soon_page'",
            "created_at": "TIMESTAMP",
        },
        "products": {
...
def backfill_coming_soon_subscribers(db) -> int:
    settings = get_website_settings_from_db(db)
    legacy_subscribers = settings.get("subscribers") if isinstance(settings.get("subscribers"), list) else []
    inserted = 0
    for raw_email in legacy_subscribers:
        email = str(raw_email or "").strip().lower()
        if not email or "@" not in email:
            continue
        exists = db.query(DBComingSoonSubscriber).filter(func.lower(DBComingSoonSubscriber.email) == email).first()
        if exists:
            continue
        db.add(DBComingSoonSubscriber(email=email, source="legacy_website_settings"))
        inserted += 1
    if inserted:
        db.commit()
    return inserted
...
def seed_database():
    try:
        ...
        if backfilled_subscribers:
            print("COMING_SOON_SUBSCRIBERS_BACKFILLED", json_dump({"count": backfilled_subscribers}))
...
def admin_debug():
    ...
    return {
        ...
        "firebase_admin_loaded": bool(firebase_admin),
        "firebase_service_account_json_loaded": bool(os.getenv("FIREBASE_SERVICE_ACCOUNT_JSON")),
        "firebase_service_account_path_loaded": bool(os.getenv("FIREBASE_SERVICE_ACCOUNT_PATH")),
        "fcm_legacy_server_key_loaded": bool(os.getenv("FCM_SERVER_KEY")),
        "firebase_app_initialized": bool(firebase_admin and firebase_admin._apps),
        "timestamp": iso(datetime.utcnow()),
    }
...
def verify_delivery_kyc_nin(payload: NINVerificationPayload, request: Request):
    ...
    try:
        ...
        if not NIN_PROVIDER_HEALTH.get("onboarding_verification_enabled", True):
            print("DELIVERY_NIN_PROVIDER_DISABLED_CONTINUING_TO_MANUAL_REVIEW", json_dump({
                "worker_id": getattr(worker, "id", None),
                "provider_status": NIN_PROVIDER_HEALTH.get("provider_status"),
                "provider_message": NIN_PROVIDER_HEALTH.get("message"),
                "timestamp": iso(datetime.utcnow()),
            }))
        if len(clean_nin) != 11:
        ...
        try:
            ...
            if error.code in provider_manual_review_codes:
                ...
                provider_result = {
                    ...
                    "nin_last4": clean_nin[-4:],
                    "raw_response": error.provider_body or {},
                    "raw_response_body": error.provider_response or "",
                    "provider_attempts": error.provider_attempts or [],
                    "failure_stage": "provider_unavailable_manual_review",
                }
                provider_data = {}
                print("NIN_PROVIDER_FALLBACK_MANUAL_REVIEW", json_dump({
                    "worker_id": worker.id,
                    "error_code": provider_error_code,
                    "provider_status": provider_http_status,
                    "nin_last4": clean_nin[-4:],
                    "message": provider_message,
                    "timestamp": iso(datetime.utcnow()),
                }))
            elif error.code == "invalid_nin":
                db.commit()
                raise HTTPException(status_code=422, detail="Invalid NIN detected. Please check the number and try again.")
            else:
                db.commit()
...
def messenger_go_online(payload: LocationPingPayload, request: Request):
    ...
    try:
        ...
        if not inside_zone and not dispatch_test_mode_enabled():
            worker.operational_status = "OFFLINE"
            db.commit()
...
def delivery_go_online(request: Request, payload: Optional[LocationPingPayload] = None):
    ...
    try:
        ...
        if normalize_worker_type(worker.worker_type) == "messenger" and not inside_zone and not dispatch_test_mode_enabled():
            worker.operational_status = "OFFLINE"
            db.commit()
            raise HTTPException(status_code=403, detail=MESSENGER_OUTSIDE_ZONE_MESSAGE)
        worker.operational_status = "ONLINE"
...
def delivery_location_ping(payload: LocationPingPayload, request: Request):
    ...
    try:
        ...
        inside_zone = update_worker_location(worker, payload, db)
        if normalize_worker_type(worker.worker_type) == "messenger" and worker.operational_status == "ONLINE" and not inside_zone and not dispatch_test_mode_enabled():
            worker.operational_status = "OFFLINE"
        db.commit()
        db.refresh(worker)
        data = worker_to_dict(worker)
...
def accept_delivery_offer(offer_id: int, request: Request):
    ...
    try:
        ...
        print("ORDER_ASSIGNMENT_FAILED", json_dump({
            "offer_id": offer_id,
            "worker_id": getattr(worker, "id", None),
            "error": repr(error),
        }))
...
def admin_get_order(order_id: int, request: Request):
    ...
    try:
        order = active_order_filter(db.query(DBOrder)).filter(DBOrder.id == order_id).first()
        if order:
            data = order_to_dict_for_context(order, db, context="admin")
            return {"success": True, "order": data, "data": data}
    finally:
        db.close()
...
@app.get("/admin/dispatch-board/diagnostics")
def admin_dispatch_diagnostics(request: Request, order_code: Optional[str] = None, order_id: Optional[int] = None, worker_id: Optional[int] = None):
    require_any_permission(request, ["orders:view", "orders:delivery", "delivery:manage", "workforce:view", "workforce:manage"])
    db = SessionLocal()
    try:
        order_query = active_order_filter(db.query(DBOrder))
        if order_id:
            order_query = order_query.filter(DBOrder.id == order_id)
        elif order_code:
            order_query = order_query.filter(func.lower(DBOrder.order_code) == str(order_code).strip().lower())
        else:
            raise HTTPException(status_code=400, detail="Provide order_code or order_id")
        order = order_query.first()
        if not order:
            raise HTTPException(status_code=404, detail="Order not found")

        if not getattr(order, "delivery_type", None) or order.delivery_type == "needs_admin_review":
            delivery_type, estimated_distance = classify_order_delivery(order, db)
        else:
            delivery_type, estimated_distance = order.delivery_type, order.estimated_distance_meters
        ready, ready_reason = delivery_order_ready_for_matching(order)
        offers = db.query(DBDeliveryOffer).filter(DBDeliveryOffer.order_id == order.id).order_by(DBDeliveryOffer.created_at.desc(), DBDeliveryOffer.id.desc()).all()
        excluded_worker_ids = {
            offer.worker_id
            for offer in offers
            if str(offer.status or "").upper() in {"DECLINED", "EXPIRED"}
        }

        workers_query = db.query(DBDeliveryWorker).filter(DBDeliveryWorker.operational_status == "ONLINE")
        if worker_id:
            workers_query = workers_query.filter(DBDeliveryWorker.id == worker_id)
        workers = workers_query.order_by(DBDeliveryWorker.last_seen_at.desc(), DBDeliveryWorker.id.asc()).all()
        worker_results = []
        for worker in workers:
            assignment_counts = worker_assignment_counts(db, worker)
            eligible, reason = worker_offer_eligibility_reason(
                db,
                worker,
                delivery_type,
                excluded_worker_ids=excluded_worker_ids,
            )
            active_offer = db.query(DBDeliveryOffer).filter(
                DBDeliveryOffer.worker_id == worker.id,
                DBDeliveryOffer.status.in_(["PENDING", "ACCEPTED", "ASSIGNED"]),
            ).order_by(DBDeliveryOffer.created_at.desc(), DBDeliveryOffer.id.desc()).first()
            worker_results.append({
                ...
                "active_order_count": assignment_counts.get("active_order_count"),
                "eligible": eligible,
                "rejection_reason": "" if eligible else reason,
                "active_offer_id": active_offer.id if active_offer else None,
                "active_offer_status": active_offer.status if active_offer else "",
            })
        ...
        diagnostic = {
            ...
            "order": {
                ...
                "assigned_rider_name": order.rider_name,
                "assigned_rider_phone": order.rider_phone,
                "delivery_type": delivery_type,
                ...
                "offer_statuses": [
                    {
                        ...
                        "worker_id": offer.worker_id,
                        "status": offer.status,
                        "assignment_status": getattr(offer, "assignment_status", ""),
                        "accepted_at": iso(offer.accepted_at),
                        "declined_at": iso(offer.declined_at),
                        "expires_at": iso(offer.expires_at),
                    }
        ...
        print("DISPATCH_DIAGNOSTIC_RESULT", json_dump(diagnostic))
        db.rollback()
        return diagnostic
    finally:
        db.close()
...
def update_order(order_id: int, payload: dict, request: Request):
    ...
    try:
        ...
        requested_payment_status = payload.get("payment_status") or (
            payload.get("status") if str(payload.get("status") or "") in ["payment_confirmed", "payment_rejected", "receipt_submitted"] else None
        )
            ...
            if (order.delivery_method or "delivery") == "delivery":
                ...
                if offer:
                    print("DISPATCH_MATCHING_GATE", json_dump({
                        ...
                        "order_code": order.order_code,
                        "ready": True,
                        "reason": "payment_confirmed_offer_created_or_existing",
                        "offer_id": offer.id,
                        "worker_id": offer.worker_id,
                    }))
                if pending_offers:
...
def delivery_worker_orders(request: Request, status: Optional[str] = None):
    ...
    try:
        ...
        print("ORDER_HISTORY_RESULT", json_dump({
            "worker_id": worker.id,
            "status_filter": clean_status,
            "returned": len(items),
            "active_order_ids": [
                item.get("id") for item in items
                if str(item.get("dispatch_status") or "").upper() in DISPATCH_ACTIVE_DELIVERY_STATUSES
            ],
            "history_order_ids": [
                item.get("id") for item in items
                if str(item.get("dispatch_status") or "").upper() in DISPATCH_HISTORY_DELIVERY_STATUSES
            ],
        }))
...
def delivery_worker_update_order_status(order_id: int, payload: DeliveryOrderStatusPayload, request: Request):
    ...
    try:
        ...
        print("DELIVERY_STATUS_SAVED", json_dump({
            "worker_id": worker.id,
            "order_id": order.id,
            "order_code": order.order_code,
            "delivery_status": order.delivery_status,
...
def get_nin_provider_diagnostics(request: Request):
    ...
    diagnostics = {
        ...
        "worker_fallback": {
            "provider_unavailable": "worker NIN is saved as KYC_PENDING/manual_review so onboarding can continue to FoodNova admin review.",
