...
4. Keep `RIDER_EARNINGS_ENABLED=false`.
5. Keep `ALLOW_STARTUP_SCHEMA_MUTATIONS=false`.
6. Deploy branch `recovery/restore-foodnova-backend`.
7. If startup reports schema drift, run `python scripts/check_schema.py`, review output, then decide whether to enable startup mutations only for staging.
