...
class DispatchRealtimeService {
  ...
  void _handleEvent(String eventName, Object? payload) {
    ...
    if (_isOfferEvent(eventName)) {
      final offer = data['offer'] is Map
          ? Map<String, dynamic>.from(data['offer'] as Map)
          : data;
      if (eventName == 'delivery_offer_created') {
        debugPrint(
          'SOCKET_DELIVERY_OFFER_RECEIVED '
          'offerId=${offer['id'] ?? data['offer_id'] ?? ''} '
          'orderId=${offer['order_id'] ?? data['order_id'] ?? ''}',
        );
      }
      debugPrint(
        'ORDER_OFFER_SOCKET_RECEIVED '
        'event=$eventName offerId=${offer['id'] ?? data['offer_id'] ?? ''} '
        'orderId=${offer['order_id'] ?? data['order_id'] ?? ''}',
      );
    }
    _events.add(data);
    ...
    if (_isOfferEvent(eventName)) {
      final offer = data['offer'] is Map
          ? Map<String, dynamic>.from(data['offer'] as Map)
          : data;
      final code = '${offer['order_code'] ?? offer['orderCode'] ?? ''}'.trim();
      DispatchNotificationService.showLocalDeliveryUpdate(
        title: 'New Delivery Request',
        body: code.isEmpty
            ? 'A new FoodNova delivery offer is available.'
            : 'FoodNova order $code is available. Accept or decline now.',
        payload: '/dashboard',
      );
    }
  ...
  bool _isOfferEvent(String eventName) {
    return eventName == 'delivery_offer_created' ||
        eventName == 'delivery:offer' ||
        eventName == 'dispatch:offer';
  }
