...
class DispatchRepository {
  ...
  Future<List<DeliveryOffer>> offers() async {
    ...
    return items
        ...
        .where((offer) {
      final status = offer.status.trim().toUpperCase();
      final assignmentStatus =
          '${offer.raw['assignment_status'] ?? offer.raw['assignmentStatus'] ?? status}'
              .trim()
              .toUpperCase();
      const incomingStatuses = {'NEW', 'PENDING', 'OFFERED'};
      return incomingStatuses.contains(status) &&
          incomingStatuses.contains(assignmentStatus);
    }).toList();
  }
  ...
  Future<List<DeliveryOrder>> history() async {
    ...
    debugPrint(
      'ORDER_HISTORY_RESULT count=${history.length} '
      'orderIds=${history.map((order) => order.id).join(',')}',
    );
    return history;
  }
