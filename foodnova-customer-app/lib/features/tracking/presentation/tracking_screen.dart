...
class _CompactTrackingSummaryCard extends StatelessWidget {
  const _CompactTrackingSummaryCard({
    required this.order,
    required this.location,
  });
  ...
  Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    final eta = order.estimatedDeliveryTime.trim().isNotEmpty
        ? order.estimatedDeliveryTime
        : 'ETA unavailable';
    final distance = location.maybeWhen(
      data: (data) => data == null || data.distanceMeters == null
          ? 'Route unavailable'
          : _formatDistance(data.distanceMeters),
      orElse: () => 'Route unavailable',
    );
    final status = _trackingStageLabel(order.canonicalDeliveryStatus);
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 10),
              Expanded(
                child: _TrackingMetricBlock(label: 'Status', value: status),
              ),
            ],
          ),
...
class _RiderTrackingCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return _CompactTrackingSummaryCard(
      order: order,
      location: location,
    );
  }
...
class _TrackingMapState extends State<_TrackingMap> {
  ...
  void initState() {
    ...
    debugPrint(
      'TRACKING_MAP_INIT '
      'rider=${widget.riderPoint.latitude},${widget.riderPoint.longitude} '
      'destination=$destinationLog '
      'polyline=${widget.routePoints.length}',
    );
  }
  ...
  Future<void> _fitBounds() async {
    ...
    final points = <LatLng>[
      ...
      if (widget.customerPoint != null) widget.customerPoint!,
      if (widget.routeDestinationPoint != null) widget.routeDestinationPoint!,
    ];
    if (points.length < 2) {
      await Future<void>.delayed(const Duration(milliseconds: 150));
      if (!mounted) return;
      debugPrint(
        'TRACKING_CAMERA_MOVE target=${widget.riderPoint.latitude},${widget.riderPoint.longitude} zoom=15',
      );
      debugPrint(
    ...
    try {
      if ((north - south).abs() < 0.0001 && (east - west).abs() < 0.0001) {
        debugPrint(
          'TRACKING_CAMERA_MOVE target=${widget.riderPoint.latitude},${widget.riderPoint.longitude} zoom=16',
        );
        debugPrint(
      ...
      debugPrint(
        'TRACKING_CAMERA_MOVE bounds=$south,$west,$north,$east padding=56',
      );
      debugPrint(
  ...
  Widget build(BuildContext context) {
    ...
    debugPrint(
      'TRACKING_MARKER_CREATED count=${markers.length} '
      'rider=${widget.riderPoint.latitude},${widget.riderPoint.longitude} '
      'destination=$destinationLog',
    );
...
class _BottomActionBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final canTrackLive = order.canTrackLive;
    return SafeArea(
