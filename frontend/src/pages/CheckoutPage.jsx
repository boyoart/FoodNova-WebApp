  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const orderData = {
        ...
        items: items.map((item) => { const quantity = item.quantity || item.qty || 1; const price = Number(item.price || item.unit_price || 0); const name = item.name || item.product_name || 'FoodNova Item'; const displayName = item.display_name || (item.variant_weight ? `${name} - ${item.variant_weight}` : name); const itemType = item.item_type || item.type || 'product'; return { id: item.id, product_id: item.product_id || item.id, variant_id: item.variant_id, variant_weight: item.variant_weight || item.weight || '', sku: item.sku || item.variant_sku || '', item_type: itemType, type: itemType, name: displayName, product_name: displayName, base_product_name: name, price, unit_price: price, quantity, qty: quantity } }),
  ...
  return (
    <div className="checkout-page"><div className="checkout-container"><div className="checkout-form"><h1>Checkout</h1><form onSubmit={handleSubmit}>
      <fieldset><legend>Delivery Method</legend><div className="delivery-options two-column-options">
        <label className={`radio-label ${deliveryMethod === 'delivery' ? 'selected' : ''}`}><input type="radio" name="deliveryMethod" value="delivery" checked={deliveryMethod === 'delivery'} onChange={(e) => setDeliveryMethod(e.target.value)} /><span className="radio-text"><strong><Truck size={17} /> Delivery</strong><small>Deliver to a saved or new address. Rider fee is paid after delivery.</small></span></label>
      ...
      <fieldset><legend>Payment Method</legend><div className="payment-info"><p><strong>Bank Transfer Details:</strong></p><p><strong>Account Number:</strong> <span className="copyable-value">6427173992 <CopyButton value="6427173992" label="Copy" /></span></p><p><strong>Bank:</strong> OPay</p><p><strong>Account Name:</strong> <span className="copyable-value">FOODNOVA LIMITED <CopyButton value="FOODNOVA LIMITED" label="Copy" /></span></p><p>Reference: Use your Order Code after placing the order.</p></div><a className="whatsapp-support-btn checkout-whatsapp-link" href={buildWhatsAppLink('Hello FoodNova, I need help placing my order.')} target="_blank" rel="noopener noreferrer"><MessageCircle size={16} />Need help? Chat with FoodNova on WhatsApp</a></fieldset><button type="submit" className="btn btn-primary btn-large" disabled={loading}>{loading ? 'Processing...' : 'Place Order'}</button>
    </form></div><div className="checkout-summary"><h2>Order Summary</h2><div className="summary-items">{items.map((item) => { const quantity = item.quantity || item.qty || 1; const price = Number(item.price || item.unit_price || 0); const label = item.display_name || (item.variant_weight ? `${item.name || item.product_name || 'FoodNova Item'} - ${item.variant_weight}` : item.name || item.product_name || 'FoodNova Item'); return <div key={item.cart_key || item.id} className="summary-item"><span>{label} x {quantity}</span><span>{formatPrice(price * quantity)}</span></div> })}</div><div className="summary-totals"><div className="summary-row"><span>Product Total:</span><span>{formatPrice(subtotal)}</span></div>{deliveryMethod === 'delivery' && <div className="summary-row"><span>Delivery Fee:</span><span className="delivery-fee">Paid to rider after delivery</span></div>}<div className="summary-row total"><span>Amount to Transfer Now:</span><span>{formatPrice(subtotal)}</span></div></div></div></div></div>
  )
}
