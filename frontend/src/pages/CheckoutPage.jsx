...
export default function CheckoutPage() {
  ...
  const [manualAddress, setManualAddress] = useState({
    ...emptyAddress,
    recipient_name: user?.name || user?.full_name || user?.fullName || '',
    phone: user?.phone || '',
  })
    ...
    const loadProfileAndAddresses = async () => {
      try {
        ...
        if (profile?.full_name || profile?.email || profile?.phone) {
          setFormData((current) => ({
            ...current,
            name: current.name || profile.full_name || profile.name || '',
            email: current.email || profile.email || '',
            phone: current.phone || profile.phone || '',
          }))
          setManualAddress((current) => ({
            ...current,
            recipient_name: current.recipient_name || profile.full_name || profile.name || '',
            phone: current.phone || profile.phone || '',
          }))
        }
  ...
  const getActiveAddress = () => {
    ...
    if (addressMode === 'saved' && selectedAddress) return selectedAddress
    return {
      ...manualAddress,
      recipient_name: manualAddress.recipient_name || formData.name,
      phone: manualAddress.phone || formData.phone,
    }
  }

  const validateAddress = (address) => {
    if (!address) return true
    return Boolean(address.state && address.city && address.address_line && address.landmark)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    if (deliveryMethod === 'delivery' && !validateAddress(activeAddress)) {
      toast.error('Please complete your delivery address: state, city, full address, and landmark are required')
      return
    }
    ...
    try {
      ...
      let deliveryAddressId = addressMode === 'saved' ? selectedAddress?.id : null

      if (deliveryMethod === 'delivery' && addressMode === 'new' && saveNewAddress && isAuthenticated) {
        try {
          const saved = await profileAPI.createAddress({
            ...activeAddress,
            recipient_name: activeAddress.recipient_name || formData.name,
            phone: activeAddress.phone || formData.phone,
            is_default: makeDefaultAddress || savedAddresses.length === 0,
          })
      ...
      const deliveryAddress = deliveryMethod === 'pickup'
        ? 'Pickup selected'
        : formatFullAddress(finalAddress)
      ...
      const orderData = {
        ...
        delivery_address_snapshot: finalAddress,
        state: finalAddress?.state || '',
        city: finalAddress?.city || '',
        lga: finalAddress?.lga || '',
        street_address: finalAddress?.address_line || finalAddress?.street || '',
        ...
        <div className="checkout-form">
          ...
          <form onSubmit={handleSubmit}>
            <fieldset>
              ...
              <div className="delivery-options two-column-options">
                <label className={`radio-label ${deliveryMethod === 'delivery' ? 'selected' : ''}`}>
                  <input
                    type="radio"
                    name="deliveryMethod"
                    value="delivery"
                  ...
                  <span className="radio-text">
                    <strong><Truck size={17} /> Delivery</strong>
                    <small>Deliver to a saved or new Nigerian address. Rider fee is paid after delivery.</small>
                  </span>
                </label>

                <label className={`radio-label ${deliveryMethod === 'pickup' ? 'selected' : ''}`}>
                  <input
                    type="radio"
                    name="deliveryMethod"
                    value="pickup"
                  ...
                  />
                  <span className="radio-text">
                    <strong><Home size={17} /> Pickup</strong>
                    <small>Pick up your order when FoodNova confirms it is ready.</small>
                  </span>
                </label>
              </div>
            </fieldset>
            ...
            <fieldset>
              ...
              <div className="form-group">
                <label><Mail size={18} /> Full Name</label>
                <input type="text" name="name" value={formData.name} onChange={handleCustomerChange} required />
              </div>

              <div className="form-group">
                <label>Email</label>
                <input type="email" name="email" value={formData.email} onChange={handleCustomerChange} required />
              </div>

              <div className="form-group">
                <label><Phone size={18} /> Phone Number</label>
                <input type="tel" name="phone" value={formData.phone} onChange={handleCustomerChange} required />
              </div>
            </fieldset>
            ...
            {deliveryMethod === 'delivery' && (
              <fieldset>
                ...
                {profileLoading && <div className="form-notice"><p>Loading saved addresses...</p></div>}
                  ...
                  <div className="saved-address-section">
                    <div className="saved-address-grid">
                      {savedAddresses.map((address) => (
                        <label
                          key={address.id}
                          className={`saved-address-card ${String(selectedAddressId) === String(address.id) ? 'selected' : ''}`}
                        >
                          <input
                            type="radio"
                            name="savedAddress"
                          ...
                          <div>
                            ...
                            <p>{address.recipient_name || formData.name} • {address.phone || formData.phone}</p>
                  ...
                  <div className="manual-address-section">
                    ...
                    <div className="form-row">
                      <div className="form-group">
                        ...
                        <input name="recipient_name" value={manualAddress.recipient_name} onChange={handleAddressChange} required />
                      </div>
                      <div className="form-group">
                        <label>Recipient Phone *</label>
                        <input name="phone" value={manualAddress.phone} onChange={handleAddressChange} required />
                      </div>
                    </div>
                    ...
                    <div className="form-row">
                      <div className="form-group">
                        ...
                        <input name="label" placeholder="Home, Office, Church" value={manualAddress.label} onChange={handleAddressChange} />
                      </div>
                      <div className="form-group">
                        <label>Country</label>
                    ...
                    <div className="form-row three-columns">
                      <div className="form-group">
                        <label>State *</label>
                        <input name="state" placeholder="e.g., Lagos" value={manualAddress.state} onChange={handleAddressChange} required />
                      </div>
                      <div className="form-group">
                        <label>City / Town *</label>
                        <input name="city" placeholder="e.g., Ikeja" value={manualAddress.city} onChange={handleAddressChange} required />
                      </div>
                      <div className="form-group">
                        <label>LGA</label>
                        <input name="lga" placeholder="e.g., Ikeja" value={manualAddress.lga} onChange={handleAddressChange} />
                      </div>
                    </div>

                    <div className="form-group">
                      <label><MapPin size={18} /> Full Address / House Number *</label>
                      <textarea
                        name="address_line"
                        placeholder="House number, street, estate, bus stop, area"
                    ...
                    <div className="form-row">
                      <div className="form-group">
                        ...
                        <input name="street" placeholder="Street, estate, area" value={manualAddress.street} onChange={handleAddressChange} />
                      </div>
                      <div className="form-group">
                        <label>Nearest Bus Stop / Landmark *</label>
                        <input name="landmark" placeholder="e.g., Near Computer Village" value={manualAddress.landmark} onChange={handleAddressChange} required />
                      </div>
                    </div>
                    ...
                    <div className="form-row">
                      ...
                      <label className="checkbox-row">
                        <input type="checkbox" checked={makeDefaultAddress} onChange={(e) => setMakeDefaultAddress(e.target.checked)} />
