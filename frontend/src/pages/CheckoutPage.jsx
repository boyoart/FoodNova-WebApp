  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const orderData = {
        ...
        items: items.map((item) => { const quantity = item.quantity || item.qty || 1; const price = Number(item.price || item.unit_price || 0); const name = item.name || item.product_name || 'FoodNova Item'; const itemType = item.item_type || item.type || 'product'; return { id: item.id, product_id: item.product_id || item.id, variant_id: item.variant_id, variant_weight: item.variant_weight || item.weight || '', sku: item.sku || item.variant_sku || '', item_type: itemType, type: itemType, name, product_name: name, price, unit_price: price, quantity, qty: quantity } }),
