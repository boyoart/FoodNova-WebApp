...
class _DashboardScreenState extends ConsumerState<DashboardScreen> {
  ...
  void initState() {
    ...
    Future<void>.microtask(() async {
      ...
      realtimeSubscription = realtime.events.listen((event) {
        final socketEvent = '${event['socket_event'] ?? ''}';
        if (socketEvent == 'delivery_offer_created' ||
            socketEvent == 'delivery:offer' ||
            socketEvent == 'dispatch:offer') {
          debugPrint(
            'OFFER_AUTO_REFRESH_TRIGGERED '
            'event=$socketEvent offerId=${event['offer_id'] ?? (event['offer'] is Map ? (event['offer'] as Map)['id'] : '')} '
            'orderId=${event['order_id'] ?? (event['offer'] is Map ? (event['offer'] as Map)['order_id'] : '')}',
          );
          debugPrint(
            'ORDER_OFFER_DASHBOARD_REFRESH '
            'event=$socketEvent offerId=${event['offer_id'] ?? (event['offer'] is Map ? (event['offer'] as Map)['id'] : '')} '
            'orderId=${event['order_id'] ?? (event['offer'] is Map ? (event['offer'] as Map)['order_id'] : '')}',
          );
        } else {
          debugPrint('DASHBOARD_REALTIME_REFRESH event=$socketEvent');
        }
        _refreshDashboardData();
        if (socketEvent == 'delivery_offer_created' ||
            socketEvent == 'delivery:offer' ||
            socketEvent == 'dispatch:offer') {
          Timer(const Duration(milliseconds: 750), _refreshDashboardData);
        }
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    ref.listen(notificationRefreshProvider, (_, __) {
      debugPrint('ORDER_OFFER_DASHBOARD_REFRESH event=notification_refresh');
    ...
    return Column(
      ...
      children: [
        ...
        offers.when(
          data: (items) {
            debugPrint(
              'DASHBOARD_OFFER_INSERTED count=${items.length} '
              'offerIds=${items.map((offer) => offer.id).join(',')}',
            );
...
class _OfferCardState extends ConsumerState<_OfferCard> {
  ...
  Widget build(BuildContext context) {
    ...
    debugPrint(
      'DASHBOARD_OFFER_RENDERED offerId=${offer.id} '
      'orderId=${offer.orderId} status=${offer.status}',
    );
