...
class _ActiveDeliveryScreenState extends ConsumerState<ActiveDeliveryScreen> {
  ...
  Future<void> _restoreLatestStatus() async {
    if (offer.orderId <= 0) return;
    try {
      final orders = await ref.read(dispatchRepositoryProvider).orders();
      final latest = orders
          .where((order) => order.id == offer.orderId)
          .cast<DeliveryOrder?>()
          .firstWhere((order) => order != null, orElse: () => null);
      if (latest == null || !mounted) return;
      final restored = DeliveryStageCopy.fromStatus(latest.status);
      debugPrint(
        'DELIVERY_STATUS_RESTORED orderId=${offer.orderId} '
        'status=${latest.status} stage=${restored.apiValue}',
      );
      if (restored != stage) {
        setState(() => stage = restored);
      }
    } catch (error) {
      debugPrint('DELIVERY_STATUS_RESTORE_FAILED $error');
    }
  }
